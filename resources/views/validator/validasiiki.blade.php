...
@section('title', 'Validasi Dokumen IKI')
@php
  $bulan = date('m');
  $tahun = date('Y');
  if($periode){
        ...
        <form method="GET" action="" class="row g-2 align-items-end mb-3">
          ...
          <div class="col-auto">
            <div class="btn-group">
              ...
              <a href="{{route('validasiiki.list')}}" class="btn btn-danger">
            ...
            <div class="table-responsive">
              <table class="table table-bordered table-hover">
                ...
                <tbody>
                  ...
                  @php
                    $ikiupload = getuploadiki([
                      'karyawan_id'=> $user->karyawan_id,
                    ]);
                    $ikis = getikiunit($user->karyawan->unit_id);
                  @endphp
                  <tr>
                    <td>
                    ...
                    <td>{{$ikis}}</td>
                    <td>{{$ikiupload->baru}} Dok</td>
                    <td>{{$ikiupload->valid}} Dok</td>
                    <td>{{$ikiupload->tidak_valid}} Dok</td>
                    <td>{{$ikiupload->total}} Dok</td>
                    <td>
                      <a href="{{route('validasiikikaryawan.list',[$user->karyawan_id])}}" class="btn btn-sm btn-primary">
